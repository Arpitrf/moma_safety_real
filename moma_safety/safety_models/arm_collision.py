...
class ArmCollisionModel():
    def __init__(self, env):  
        '''MODEL LOADING'''
        num_class = 1
        # experiment_dir = "/home/pal/arpit/Pointnet_Pointnet2_pytorch/pointnet2/log/classification/run_place_in_shelf"
        experiment_dir = "/home/pal/arpit/Pointnet_Pointnet2_pytorch/pointnet2/log/classification/run_place"

        self.classifier = action_pointnet2_cls_ssg.get_model(num_class, normal_channel=False)
        self.classifier = self.classifier.cuda()

        checkpoint = torch.load(str(experiment_dir) + '/checkpoints/model_epoch_200.pth')
    ...
    def batched_predict_collisions(self, action, threshold=0.5, object_name=None, inference_batch_size=5):
        points = self.points
        # actions = actions.unsqueeze(0)
        # action_rotvec = R.from_quat(action[6:10]).as_rotvec()
        # if action[-1] == 0:
        #     gripper_action = -1.0
        # else:
        #     gripper_action = 1.0
        # action = np.concatenate((action[:6], action_rotvec, np.array([gripper_action])))
        # actions = action[None, ...]
        ...
        for i in range(len(action)):
            action_rotvec = R.from_quat(action[i][6:10]).as_rotvec()
            
            if action[i][-1] == 0:
                gripper_action = -1.0
            else:
                gripper_action = 1.0
            final_action = np.concatenate((action[i][:6], action_rotvec, np.array([gripper_action])))
            actions.append(final_action)
        actions = torch.from_numpy(np.array(actions)).to(dtype=torch.float64)
        # ================================
            
        # points_new = points.repeat(5,1,1)
        # actions_new = actions.repeat(5,1)
        # pred, _ = self.classifier(points_new, actions_new)
        ...
        # repeat points
        points = points.repeat(inference_batch_size,1,1)
        ...
        for i in range(inference_batch_size):
            print(f"pred_choice, pred_prob: ", pred_choice[i].item(), probabilities[i].item(), action[i][3:6])
        # -----------------------------

        return pred_choice.cpu().numpy(), probabilities.detach().cpu().numpy()
    
    def predict_collisions(self, action, threshold=0.5, object_name=None):
        points = self.points
        # actions = actions.unsqueeze(0)
        action_rotvec = R.from_quat(action[6:10]).as_rotvec()
        if action[-1] == 0:
