...
def move_arm_back(env, obj_name):
    ...
    if info["arm_right"]["joint_goal"] is None:
        return False
    return True


def move_arm_to_held_pose(env, obj_name, gripper_state="closed", delay_scale_factor=4.0, z_pos_lis = [1.2, 1.1, 1.0, 0.9]):
    final_actions = []
    current_right_ee_pose = env.tiago.arms["right"].arm_pose
...
class TaskExploration:
    def __init__(self, env):
        ...
        with open('resources/human_tracking_outputs/arpit_fill_water_in_pot_temp.json') as f:
    ...
    def cleanup_post_manip(self, object_name):
        ...
        # 1. Move arm back
        action_exec = move_arm_back(self.env, object_name)
        if not action_exec:
            move_arm_to_held_pose(self.env, object_name, gripper_state="open", delay_scale_factor=8.0, z_pos_lis = [0.8, 0.7, 0.6, 0.55])
        ...
        reset_joint_pos = RP.PREGRASP_HIGH_SINK
        ...
        breakpoint()
        self.env.reset(reset_arms=True, reset_pose=reset_joint_pos, delay_scale_factor=10.0) 
    ...
    def expl(self, subtask_idx):
        ...
        if "grasp" in subtask["description"].lower():
            ...
            while self.grasp_mode_idx < len(self.grasp_modes) and should_explore_grasp:
                ...
                retval = execute_grasp(self.env, grasp_mode, object_name, switch_to_impedance_controller=OC[object_name]["use_impedance_controller"])
...
def main():
    ...
    env.reset(reset_arms=True, reset_pose=reset_joint_pos, delay_scale_factor=4.0)
