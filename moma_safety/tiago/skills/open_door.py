...
""".strip()
    ...
    def get_param_from_response(self, response, info):
        ...
        error_list = []
        try:
            direction = vip_utils.extract_json(response, 'place_to_push')
    ...
    def align_with_door(self, env, floor_num, arm='right', execute=True):
        ...
        if floor_num == 1:
            # ori_map = np.asarray([0.0, 0.0, 0.6476465062154938, 0.7619409445533586])
        ...
        # assert state == GoalStatus.SUCCEEDED, f"Failed to reach the approach position. State: {state}"
    ...
    def step(self, env, rgb, depth, pcd, normals, query, arm='right', execute=True, run_vlm=True, info=None, **kwargs):
        ...
        # goal_pos_base = np.asarray([door_distance + self.goal_dist, 0.0, 0.0])
        ...
        if execute:
            ...
            state = self.send_move_base_goal(goal)
            is_success = True
            # assert state == GoalStatus.SUCCEEDED, f"Failed to reach the goal position. State: {state}"
            if text_direction == 'left':
