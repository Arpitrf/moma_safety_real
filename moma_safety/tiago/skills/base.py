...
from moma_safety.tiago.ros_restrict import set_init_pose
...
def movebase_code2error(status_code):
    status_dict = {
        ...
        GoalStatus.PREEMPTED: ('ABORTED', 'The goal was aborted during execution by the action server due to collision at the goal pose.'), #('PREEMPTED', 'The goal received a cancel request after it started executing and has since completed its execution (the goal was preempted).'),
...
class SkillBase:
    def __init__(
        ...
        # self._gripper_correction_vec = np.array([0.19, 0.03, -0.02]) # this is the value used for wiping but is incorrect
        ...
        assert method in ["ours", "llm_baseline", "ours_no_markers", "distance", "full_teleop"]
        self.method = method
        # skill selection. Our method will have this as False.
    ...
    #         print("Should not be here"); import pdb; pdb.set_trace()
    ...
    def get_object_bboxes(self, rgb, query, gsam=None):
        # Timing for the entire function
        start_total = time.time()

        # Start timing for the initialization
        ...
        if (self._gsam is None) and (gsam is None):
            #print("Should not be here"); import pdb; pdb.set_trace()
            # self._gsam = GroundedSamWrapper(sam_ckpt_path=os.environ['SAM_CKPT_PATH'])
            self._gsam = GroundedSam2Wrapper()
        if gsam is None:
            gsam = self._gsam

        # End timing for initialization
        end_init = time.time()
        print(Fore.CYAN + f"Initialization took: {end_init - start_init:.4f} seconds")
        ...
        # End timing for segmentation
        end_segmentation = time.time()
        print(Fore.GREEN + f"Segmentation took: {end_segmentation - start_segmentation:.4f} seconds")

        # Start timing for bounding box extraction
        start_bbox_extraction = time.time()

        # Process bounding boxes from the mask
        for label in unique_labels:
            mask = final_mask_image == label
            # Get the bbox of the mask
            nonzero = np.nonzero(mask)
        ...
        # End timing for bounding box extraction
        end_bbox_extraction = time.time()
        print(Fore.YELLOW + f"Bounding box extraction took: {end_bbox_extraction - start_bbox_extraction:.4f} seconds")

        # End timing for the entire function
        end_total = time.time()
        print(Fore.MAGENTA + f"Total time for get_object_bboxes: {end_total - start_total:.4f} seconds")
    ...
    def goto_odom_pos(self, env, pos, threshold=0.02, speed=0.5):
        ...
        # print(diff, np.any(diff) > threshold)
...
ANSWER: Let's think step-by-step."""
    ...
    def vlm_runner(
        ...
        if self.method == "llm_baseline" and not force_vlm_prompt:
            ...
            instructions, task_prompt = make_prompt_func(**make_prompt_func_kwargs)
            if encoded_image_joystick is not None:
                prompt_seq = [task_prompt, encoded_image, encoded_image_joystick]
            else:
                prompt_seq = [task_prompt, encoded_image]
        if self.method != "ours":
    ...
    def vlm_runner_video(
        ...
        if False: # self.method == "llm_baseline" and not force_vlm_prompt:
            instructions, prompt_seq = self.prep_llm_prompt(
                encoded_image,
                encoded_image_joystick,
                make_prompt_func,
            ...
            instructions, task_prompt = make_prompt_func(**make_prompt_func_kwargs)
            prompt_seq = [task_prompt]
            for encoded_image in encoded_image_lst:
                prompt_seq.append(encoded_image)

        if self.method != "ours":
            print(colored(f"{self.method} Prompt\n" + instructions + task_prompt, 'light_blue'))
        response = self.vlm.query(instructions, prompt_seq, history=history_msgs, parallel=False, num_parallel=5)
