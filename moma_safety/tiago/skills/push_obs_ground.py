...
""".strip()
    ...
    def get_history_dirs(self):
        # base_dir = "/home/pal/Desktop/rutav/datasets/push_obs_ground_3/prompt_data_elev_oracle"
        # history_eval_dirs = [os.path.join(base_dir, 'eval_id001.pkl')]

        base_dir = "/home/pal/Desktop/rutav/datasets/push_obs_ground_3/prompt_data_elev_model"
        history_eval_dirs = [os.path.join(base_dir, 'eval_id004.pkl')]

        base_dir = "/home/pal/Desktop/rutav/datasets/push_chair_inside/prompt_data_elev_model"
        history_eval_dirs.extend([os.path.join(base_dir, 'eval_id001.pkl')])
    ...
    def get_dir_from_response(self, response, info):
        ...
        error_list = []
        try:
            direction = vip_utils.extract_json(response, 'direction_to_push')
    ...
    def step(self, env, rgb, depth, pcd, normals, query, execute=True, run_vlm=True, info=None, bboxes=None, mask_image=None, history=None, **kwargs):
        ...
        if text_direction == 'forward':
            ...
            # approach_pos2 = np.array([np.min(obj_pos_f[:,0]) + 1.30, y_pos, 0.0]) # be 130 cm into the object
            # approach_pos3 = approach_pos2.copy()
            # goto_pos = np.array([np.min(obj_pos_f[:,0]) + 1.0, y_pos, 0.0]) # come back by 1m into the object
            # goto_pos = np.array([x_pos, y_pos, 0.0])
            # goto_pos = copy.deepcopy(approach_pos1)
            ...
            # align_pos_odom, approach_pos1_odom, approach_pos2_odom are the same.
            # go to the right most position
            # push_pull_arm is skipped, instead we extend the left arm and rotate the arm to different joint angle
            # bring back the arm to original position
            # use goto_pos_map, goto_ori_map
            ext_x_pos = np.min(obj_pos_f[:,0])
            align_pos_base = np.array([ext_x_pos - distance4hand_x, ext_right_pos_y - distance4hand_y, 0.0])
            approach_pos1 = align_pos_base
            approach_pos2 = align_pos_base
            # approach_ori2 = np.asarray([0.0, 0.0, 0.7071068, 0.7071068])
            # arm_joint_angle_series = [
            #     [-0.00, -0.0, 1.718, 0.083, -1.404, -0.112, 0.157],
            #     [1.50, -0.0, 1.718, 0.083, -1.404, -0.112, 0.157],
            #     [-1.107, 1.466, 2.703, 1.718, -1.414, 1.391, 0.002],
            # ]
            approach_ori2 = np.asarray([0.0, 0.0, 0.0, 1.0])
            arm_joint_angle_series = [
        ...
        if env is not None:
            ...
            # approach_pos3_odom = transform @ np.concatenate((approach_pos3, [1.0]))
            # approach_pos3_odom = approach_pos3_odom[:3]
        ...
        if execute:
            ...
            if text_direction == 'forward':
                ...
                # self.push_pull_arm(env, arm, goto_pos=None)
                ...
                for joint_angles in arm_joint_angle_series:
                    # print("Continue to next joint angle place?")
                    # input("Press any key to continue")
