...
"""
        ...
        execute = True#U.confirm_user(execute, "Do you want to continue? (y/n): ", "Picking up the object.")
        if not execute:
            return self.on_failure(
                env=env,
                arm=arm,
                reason_for_failure=f"Human decided to not execute the subtask: {query}",
                reset_required=False,
                capture_history=capture_history,
                return_info=return_info,
                open_gripper=False,
        ...
        if execute:
            ...
            # Hack to determine grasping success
            gripper_dist = env.tiago.gripper[arm].get_state()
            if gripper_dist < 0.03: # not grasping anything
                error = "Failing to pick up the object, missing the grasp."
                success = False
