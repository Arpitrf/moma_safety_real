...
# You are a five-time world champion in this game. Output only one of the directions: forward, backward, left, or right. Do NOT leave it empty. First, summarize all the errors made in previous predictions if provided. Then, briefly describe the scene in the image, explain where the objects are placed. Using the distance information provided provide an explicit verification whether moving forward will lead to collision or not. Use the information of the nearest object, example,
# - The nearest object is 0.4 meters away from the robot. 0.4 > {info['move_dist']}. However, the objects are on the table. The edge of the table will collide with the robot's body (not visible) if the robot moves in forward directioin. Moving forward will lead to a collision of the robot with the table.
# - The nearest object is 1.0 meters away from the robot. Since 1.0 > {info['move_dist']} and there is no other obstalce in the environment restricting the robot's movement in the forward direction, moving forward will NOT lead to a collision with any object in the scene.
# Then, {visual_instructions[3]}describe the effect of the robot moving in each direction. {visual_instructions[4]}Then, select the direction that can best help complete the task of reaching near the object of interest. Finally, provide the direction in a valid JSON of this format:
# {{"direction_to_move": ""}}
# """.strip()
...
class MoveToSkill(SkillBase):
    def __init__(
        ...
        if self.add_histories:
            ...
            # base_dir = '/home/pal/Desktop/rutav/vlm-skill/experiments/dataset_for_movebase/histories_oracle_move/'
            # history_eval_dirs = [os.path.join(base_dir, 'eval_id001.pkl'), os.path.join(base_dir, 'eval_id003.pkl'), os.path.join(base_dir, 'eval_id004.pkl'), os.path.join(base_dir, 'history_forward.pkl')]
            # base_dir = '/home/pal/Desktop/rutav/vlm-skill/experiments/dataset_for_movebase/histories_model_move/'
            # history_eval_dirs = [os.path.join(base_dir, 'eval_id001.pkl'), os.path.join(base_dir, 'eval_id003.pkl'), os.path.join(base_dir, 'eval_id004.pkl'), os.path.join(base_dir, 'history_forward.pkl')]
            base_dir = '/home/pal/Desktop/rutav/vlm-skill/experiments/movebase_new_data/histories_model_move/'
            history_eval_dirs = [os.path.join(base_dir, 'eval_id001.pkl'), os.path.join(base_dir, 'history_forward.pkl'), os.path.join(base_dir, 'eval_id003.pkl')]
            # history_eval_dirs = [os.path.join(base_dir, 'eval_id001.pkl'), os.path.join(base_dir, 'eval_id003.pkl'), os.path.join(base_dir, 'eval_id004.pkl')]
            history_list = []
...
""".strip()
    ...
    def get_param_from_response(self, response, info):
        ...
        error_list = []
        try:
            direction = vip_utils.extract_json(response, 'direction_to_move')
        ...
        if run_vlm:
            ...
            if self.add_obj_id: # add object ids to the image
                ...
                for bbox in bboxes:
                    if bbox[0] == 0:
                        continue
                    _id = [_bbox.obj_id for _bbox in obj_bbox_list if _bbox.env_id == bbox[0]][0]
                    mask = mask_image == bbox[0]
                    # make the mask of the same size as the pcd
                    pcd_masked = pcd[mask]
                    # remove all nan values
                    pcd_masked = pcd_masked[~np.all(np.isnan(pcd_masked), axis=1)]
                    if pcd_masked.shape[0] != 0:
                        mean_pos = np.mean(pcd_masked, axis=0)
                        bbox_id2dir[_id] = {}
                        if mean_pos[0] > 0:
                            bbox_id2dir[_id].update({'forward': mean_pos[0]})
                        elif mean_pos[0] < 0:
                            bbox_id2dir[_id].update({'backward': -mean_pos[0]})

                        if mean_pos[1] > 0:
                            bbox_id2dir[_id].update({'left': mean_pos[1]})
                        elif mean_pos[1] < 0:
                            bbox_id2dir[_id].update({'right': -mean_pos[1]})
                    else:
                        bbox_id2dir[_id] = None
                info_cp['bbox_id2dir'] = bbox_id2dir
                '''
