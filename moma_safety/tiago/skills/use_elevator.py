...
""".strip()
    ...
    def get_history_dirs(self):
        ...
        # history_eval_dirs = [os.path.join(base_dir, 'eval_id019.pkl')]
        ...
        base_dir = '/home/pal/Desktop/rutav/datasets/test_elevator_mbb_north_out2/prompt_data_elev_floor1_model_test/'
    ...
    def step(self, env, rgb, depth, pcd, normals, arm, query, execute=True, run_vlm=True, info=None, **kwargs):
        ...
        if execute:
            ...
            # assert state == GoalStatus.SUCCEEDED

            # execute = U.confirm_user(execute, "Do you want to continue? (y/n): ", "Rotating inside the elevator.")
            # if not execute:
            #     return False, "Execution failure", capture_history, return_info
            # approach_pose = self.get_base_approach_pose_map(floor_num, bld=kwargs['bld'])
            # approach_in_pose = self.get_base_approach_in_pose_map(floor_num, bld=kwargs['bld'])
            # rotate_pose = (approach_pose[0], approach_in_pose[1])
            # print(rotate_pose)
            # goal = self.create_move_base_goal(rotate_pose)
            # print(goal)
            # state = self.send_move_base_goal(goal)

            # execute = U.confirm_user(execute, "Do you want to continue? (y/n): ", "Moving inside the elevator final pose.")
            # if not execute:
            #     return False, "Execution failure", capture_history, return_info
        ...
        # this will return the updated history!
        return success, reason_for_failure, capture_history, return_info
...
""".strip()
    ...
    def step(self, env, rgb, depth, pcd, normals, arm, query, execute=True, run_vlm=True, info=None, **kwargs):
        ...
        rospy.sleep(2)
        # if everything is successful, then return then change the floor_num if 1 then to 2. if 2 then to 1.
        if success:
            # return_info['floor_num'] = 1 if floor_num == 2 else 2
            if run_vlm:
