...
class PlaceSkill(SkillBase):
    def __init__(
        ...
        self.debug = debug
        self.vis_dir = os.path.join(run_dir, 'place')
...
""".strip()
    ...
    def get_param_from_response(self, response, obj_bbox_list, pcd, mask_image, execute):
        ...
        poses = poses[~mask]
        if len(poses) == 0:
            error = f"Couldn't place the object."
            return_info['error_list'].append(error)
            return None, object_id, return_info
        # highest point (z) is the object
        highest_z_val = np.max(poses[:, 2])
        # self.placing_height_offset = 0.04
        # highest_z_val = np.max(poses[:, 2]) + self.placing_height_offset

        # x value should be the minimum x value of the object
        x_val = np.mean(poses[:, 0])
        # x_val = np.mean([np.min(poses[:, 0]), np.max(poses[:, 0])])
    ...
    def on_failure(self, env, arm, *args, **kwargs):
        ...
        '''
        self.reset_arm_to_original_pose(env, arm, duration_scale_factor=3.0)
        if env:
    ...
    def step(self, env, rgb, depth, pcd, normals, arm, query, execute=True, run_vlm=True, info=None, bboxes=None, mask_image=None, **kwargs):
        ...
        if run_vlm:
            ...
            bbox_id2dist = {}
            bbox_id2eefdist = {} # useful for the distance baseline
            eef_pos_wrt_base, eef_ori_wrt_base = self.tf_base.get_transform(f'/gripper_{arm}_grasping_frame')
            for bbox in bboxes:
                center = (bbox[1] + bbox[3]) // 2, (bbox[2] + bbox[4]) // 2
                pos_wrt_base = pcd[center[1], center[0]]
                bbox_id2eefdist[bbox[0]] = np.linalg.norm(pos_wrt_base[:3] - eef_pos_wrt_base[:3])
            ...
            if self.method == "distance":
                bbox_id2eefdist.pop(0, None)
                if len(bbox_id2eefdist.keys()) == 0:
                ...
                # find the id with the minimum distance from bbox_id2eefdist
                object_id = min(bbox_id2eefdist, key=bbox_id2eefdist.get)
                prob = 1/(1+5*bbox_id2eefdist[object_id])
                # get the next mini
                object_id_next = min([k for k in bbox_id2eefdist if k != object_id], key=bbox_id2eefdist.get)
                prob_next = 1/(1+5*bbox_id2eefdist[object_id_next])
...
"""
        ...
        execute = True #U.confirm_user(execute, "Do you want to continue? (y/n): ", "Placing the object.")
        if not execute:
            return self.on_failure(
                env=env,
                arm=arm,
        ...
        if execute:
            ...
            obs, reward, done, info = self.arm_goto_pose(pose=(approach_pos, approach_ori), n_steps=2, duration_scale_factor=duration_scale_factor, gripper_act=np.array([0.0]), **goto_args)
            ...
            obs, reward, done, info = self.arm_goto_pose(pose=(goto_pos_base, goto_ori_base), n_steps=2, duration_scale_factor=2*duration_scale_factor, gripper_act=np.array([0.0]), **goto_args) # move twice as slow
            env.tiago.gripper['left'].step(0.0)
            ...
            if success:
                ...
                print("IK FAILED; STOP EXECUTION")
