...
""".strip()
    ...
    def get_param_from_response_obj(self, response, obj_bbox_list, pcd, mask_image):
        ...
        if len(poses) == 0:
            error = f"Depth values for the selected object are all nan. Please try again."
            return_info['error_list'].append(error)
            return None, object_id, return_info
        mean_pos = np.mean(poses, axis=0)
        min_x_pos = np.min(poses[:, 0])
        return_info['min_x_pos'] = min_x_pos
        # if nan, then we need to find points that are not nan and are near the object.
        return mean_pos, object_id, return_info

    def look_down_and_collect_points(self, env, fav_pos, return_info):
        self.send_head_command(head_positions=[0.0, -0.9]) # hard-coded to look down
        obs_pp = VU.get_obs(env, self.tf_base)
        rgb_down, depth_down, _, _, pcd_down, normals_down = obs_pp['rgb'], obs_pp['depth'], obs_pp['cam_intr'], obs_pp['cam_extr'], obs_pp['pcd'], obs_pp['normals']
        pcd_ground_down, rgb_ground_down = self.create_interested_ground_points(pcd_down, rgb_down, fav_pos, return_info)
        # mask_ground_down = pcd_down[:, :, 2] < 0.05
        # mask_interest_down = np.linalg.norm(pcd_down[:, :, :2] - fav_pos.reshape(-1,3)[:, :2], axis=-1) < self.fav_pos_interest_th
        # _mask_down  = np.logical_and(mask_ground_down, mask_interest_down)
        # pcd_ground_down = pcd_down[_mask_down]
        # rgb_ground_down = rgb_down[_mask_down]
        # pcd_ground_down, rgb_ground_down = U.remove_nan_pcd(pcd_ground_down, rgb_ground_down)
        self.send_head_command(head_positions=self.default_head_joint_position)
        return pcd_ground_down, rgb_ground_down

    def create_interested_ground_points(self, pcd, rgb, fav_pos, return_info):
        mask_ground = pcd[:, :, 2] < 0.05
        mask_interest = np.linalg.norm(pcd[:, :, :2] - fav_pos.reshape(-1,3)[:, :2], axis=-1) < self.fav_pos_interest_th
        min_x_pos = return_info['min_x_pos']
        infront_obj_mask = pcd[:, :, 0] < min_x_pos - 0.1
        favpos_y_mask = np.logical_and(pcd[:, :, 1] < fav_pos[1] + 0.05, pcd[:, :, 1] > fav_pos[1] - 0.05)
        mask = np.logical_and(mask_ground, mask_interest)
        mask = np.logical_and(mask, infront_obj_mask)
        mask = np.logical_and(mask, favpos_y_mask)
        pcd_ground = pcd[mask]
        rgb_ground = rgb[mask]
        pcd_ground, rgb_ground = U.remove_nan_pcd(pcd_ground, rgb_ground)
        return pcd_ground, rgb_ground

    def sample_candidate_pts(self, env, pcd, rgb, fav_pos, return_info):
        pcd_ground = pcd.copy()
        rgb_ground = rgb.copy()
        ...
        '''
        pcd_ground, rgb_ground = self.create_interested_ground_points(pcd_ground, rgb_ground, fav_pos, return_info)
        if pcd_ground.shape[0] == 0:
            # if there are no ground points, then we collect more points by sending head command.
            pcd_ground_down, rgb_ground_down = self.look_down_and_collect_points(env, fav_pos, return_info)
            pcd_ground = np.concatenate((pcd_ground, pcd_ground_down), axis=0)
            rgb_ground = np.concatenate((rgb_ground, rgb_ground_down), axis=0)
            print("After looking down: ", pcd_ground.shape)
            if pcd_ground.shape[0] == 0:
                print("No points found on the ground.")
                return None
        ...
        bs1 = 32768
        # example shapes: (92116, 3) (3954, 3)
        # dist = np.linalg.norm(np.expand_dims(pcd_ground[:,:2], axis=1) - np.expand_dims(pcd_non_ground[:,:2], axis=0), axis=-1) # this is very expensive and slow. Optimize this batch operation and chunkin
        ...
        # keep only the points that are atleast 60 cm away from the non-ground points
        # import ipdb; ipdb.set_trace()
        keep_dist = 0.35
        mask = np.all(dist > keep_dist, axis=1)
        if np.sum(mask) == 0:
            pcd_ground_down, rgb_ground_down = self.look_down_and_collect_points(env, fav_pos, return_info)
            print("After looking down: ", pcd_ground_down.shape, pcd_non_ground.shape)
            # dist_down = np.linalg.norm(np.expand_dims(pcd_ground_down[:,:2], axis=1) - np.expand_dims(pcd_non_ground[:,:2], axis=0), axis=-1)
            dist_down = np.zeros((pcd_ground_down.shape[0], pcd_non_ground.shape[0]), dtype=np.float32)
            for i in range(0, pcd_ground_down.shape[0], bs1):
                dist_down[i:i+bs1] = np.linalg.norm(np.expand_dims(pcd_ground_down[i:i+bs1,:2], axis=1) - np.expand_dims(pcd_non_ground[:,:2], axis=0), axis=-1)
            mask_down = np.all(dist_down > keep_dist, axis=1)
            pcd_ground = np.concatenate((pcd_ground, pcd_ground_down), axis=0)
            rgb_ground = np.concatenate((rgb_ground, rgb_ground_down), axis=0)
            dist = np.concatenate((dist, dist_down), axis=0)
            mask = np.concatenate((mask, mask_down), axis=0)
        ...
        min_x_pos = return_info['min_x_pos']
        mask = pcd_ground[:, 0] < min_x_pos - 0.1
        # sample pcd_ground that are 5 +/- from the fav_pos y value
        mask = np.logical_and(mask, np.logical_and(pcd_ground[:, 1] < fav_pos[1] + 0.05, pcd_ground[:, 1] > fav_pos[1] - 0.05))
        pcd_ground = pcd_ground[mask]
        rgb_ground = rgb_ground[mask]
        ...
        i = np.argmin(dist)
        pt = pcd_ground[i]
        # min_dist_mask, all the points that are at the minimum distance by 3 cm in x, y
        ...
        # rgb_selected_pts = np.asarray([[255.0, 0.0, 0.0]*selected_pts.shape[0]]).reshape(-1,3)
        # pcd_to_plot = np.concatenate((pcd.reshape(-1, 3), selected_pts), axis=0)
        # rgb_to_plot = np.concatenate((rgb.reshape(-1, 3), rgb_selected_pts), axis=0)
    ...
    def step(self, env, rgb, depth, pcd, normals, query, execute=True, run_vlm=True, info=None, bboxes=None, mask_image=None, **kwargs):
        ...
        if env is not None:
            ...
            # reset arm first, to get a nice view
            arm = "left" # hardcoded
            grasp_h_r = RP.PICKUP_TABLE_L_HOME_R
            grasp_h_r[arm][-1] = env.tiago.gripper[arm].get_state()

            self.original_joint_angles = env.tiago.arms[arm].joint_reader.get_most_recent_msg()
            # self.original_joint_angles = np.asarray(RP.PICKUP_TABLE_L_HOME_R_VIS[arm][:-1])
            name = 'pickup_table_l_home_r'
            grasp_h_r['torso'] = None
            U.reset_env(env, reset_pose=grasp_h_r, reset_pose_name=name, delay_scale_factor=1.0)
        ...
        if run_vlm:
            ...
            if self.method == "distance":
                bbox_id2dist = copy.deepcopy(info_cp['bbox_id2dist'])
                # remove the object with id 0
                bbox_id2dist.pop(0, None)
                if len(bbox_id2dist.keys()) == 0:
                    return self.on_failure(
                        reason_for_failure=f"No objects found.",
                        reset_required=False,
                        capture_history={},
                        return_info={},
                    ) 
                # get the minimum distance object fromt bbox_id2dist, ignore the object with id 0
                object_id = min(bbox_id2dist, key=bbox_id2dist.get)
                prob = 1/(1+5*bbox_id2dist[object_id])
                object_id_next = min([k for k in bbox_id2dist if k != object_id], key=bbox_id2dist.get)
                prob_next = 1/(1+5*bbox_id2dist[object_id_next])
                confidence = prob - prob_next
                final_obj_id = [bbox.obj_id for bbox in obj_bbox_list if bbox.env_id == object_id][0]
                response = f"""
```json
...
"""
            ...
            selected_pts = self.sample_candidate_pts(env, pcd, rgb, fav_pos, return_info)
        ...
        is_success = False
        error = None
        execute = True #U.confirm_user(execute, "Do you want to continue? (y/n): ", f"Using the navigate_to_point_on_ground skill for {text_direction} direction")
        if execute:
            goal = self.create_move_base_goal((goal_pos_map, goal_ori_map))
            state = self.send_move_base_goal(goal)
            # if state != GoalStatus.SUCCEEDED:
            #     print(f"COLLISION AT GOAL STATE! COLLISION AT GOAL STATE!\nMove base failed with state: {state}")
            if state == GoalStatus.SUCCEEDED:
                is_success = True
            else:
        ...
        # reset arm, regardless of success
        self.reset_arm_to_original_pose(env, arm="left", duration_scale_factor=3.0)
    ...
    def reset_arm_to_original_pose(self, env, arm, duration_scale_factor=1.0):
        duration_scale_factor = 3.0
        cur_joint_angles = env.tiago.arms[arm].joint_reader.get_most_recent_msg()
        duration_scale = 1.5 * np.linalg.norm(self.original_joint_angles-cur_joint_angles)
        env.tiago.arms[arm].write(self.original_joint_angles, duration_scale, delay_scale_factor=duration_scale_factor)
        return
    ...
    def on_success(
        ...
        *args, **kwargs
        ...
        self.send_head_command([0.0, -0.8])
    ...
    def on_failure(
        ...
        *args, **kwargs
        ...
        self.send_head_command([0.0, -0.6])
