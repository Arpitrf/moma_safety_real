...
# def make_prompt_for_reasoning(desc, feedback):
#     instructions = f"""
# You will be given an image with a point marked on it. The point indicates the location you selected that the robot must navigate to to complete the task. Along with the image, you are provided with the description of the task. After executing the task, we got the feedback from the environment. The feedback is in the form of an indication of success or failure. If there was a failure, you are provided with the environment feedback briefly explaining the reason for the failure. Reason about the selected point, task description, and the feedback from the environment to provide a short analysis of failure to avoid such mistakes in the future. Provide your answer at the end in a valid JSON of this format: {{"short_analysis": []}}. Think step by step and finally provide your analysis in the JSON format.

# An example of such an answer in JSON format could be: {{"short_analysis": ["The point was too close to the object. The robot's base is large so the point should be at a distance from the objects."]}}
# """.strip()
...
class Reasoner(SkillBase):
    ...
    def get_param_from_response(self, response, query, info):
        ...
        try:
            analysis = vip_utils.extract_json(response, 'short_analysis')
