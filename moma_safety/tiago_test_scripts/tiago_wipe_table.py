...
from moma_safety.tiago.skills.wipe_table import WipeTableSkill
...
def main(args):
    ...
    wipe_table = WipeTableSkill(oracle_position=True, debug=args.debug)
    tf_listener = TFTransformListener('/base_footprint')
    grasp_h_r = RP.WIPE_TABLE_L
    ...
    wipe_table.step(env, rgb, depth, pcd=pcd, normals=normals, arm=args.arm, execute=args.exec)
