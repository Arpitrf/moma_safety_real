...
from moma_safety.tiago.skills.pull_drawer import PullDrawerSkill
...
def main(args):
    ...
    pull_drawer = PullDrawerSkill(oracle_position=True, debug=args.debug)
    tf_listener = TFTransformListener('/base_footprint')
    grasp_h_r = RP.PULL_DRAWER_L
    ...
    pull_drawer.step(env, rgb, depth, pcd=pcd, normals=normals, arm=args.arm, execute=args.exec)
