...
def reset_env(
    ...
    reset_pose = copy.deepcopy(reset_pose)
    ...
    user_input = True
    if not skip_reset:
        user_input = True #confirm_user(True, 'Move to reset pose? (y/n): ') # if the user says no, then we will not move to the reset pose. Simply return False
    if not user_input:
        return False
    if int_pose is not None:
...
if __name__ == '__main__':
    ...
    log['a'] = [1, 2, 3]
    log['b'] = [4, 5, 6]
    save_csv(log, 'test.csv')
    log = load_csv('test.csv')
    print(log)
...
def get_wandb_logger(wandb_project, wandb_run, wandb_dir, cfg):
    num_attempts = 10
    import wandb
    for attempt in range(num_attempts):
        try:
            # set up wandb
            wandb.init(project=wandb_project, dir=wandb_dir, resume=False, config=cfg, mode="offline" if attempt == num_attempts-1 else cfg.wandb_mode, name=wandb_run)
            break
